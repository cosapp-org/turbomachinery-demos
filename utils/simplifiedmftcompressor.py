...
class SimplifiedMftCompressor:
    ...
    def wr(self, pcnr, gh):
        vlsl = self.velocity(gh) * self._utrd * pcnr / 100.
        wr = self._inlet_area * (self.Pstd / (self.Tstd * self._r)) * vlsl * (1 - vlsl**2 / (2 * self._cp * self.Tstd))**(1 / (self._gamma - 1))
        return wr

    def pr(self, pcnr, gh):
        ghri = self._psi + gh - self.loss(gh)
        tqi = (self._utrd * pcnr / 100)**2 * ghri / (2. * self._cp * self.Tstd) + 1.
        return tqi**(self._gamma / (self._gamma - 1.))
    ...
    def velocity(self, gh):
        if gh < self._gh_vqu_sat:
            ...
            return self._vqu_sat * (1. - np.log((np.exp(self._kshape) - 1.) * x**self._exps + 1) / self._kshape)
